...
{
    const auto [minMixin, maxMixin, defaultMixin] = mevacoin::Mixins::getMixinAllowableRange(height);
    ...
    const uint64_t currentHeight)
{
    ...
    if (fee < mevacoin::parameters::MINIMUM_FEE)
    {
    ...
    const bool integratedAddressesAllowed)
{
    for (auto &address : addresses)
    {
        ...
        if (address.length() == wallet_config::integratedAddressLength)
        {
            ...
            address = mevacoin::getAccountAddressAsStr(
        ...
        /* Not used */
        mevacoin::AccountPublicAddress ignore2;
