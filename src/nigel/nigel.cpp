...
bool Nigel::getDaemonInfo()
{
    ...
    if (res && res->status == 200)
    {
        try
        {
            ...
            m_lastKnownHashrate = j.at("difficulty").get<uint64_t>() / mevacoin::parameters::DIFFICULTY_TARGET;
...
std::tuple<bool, std::vector<mevacoin::RandomOuts>> Nigel::getRandomOutsByAmounts(
    ...
    const uint64_t requestedOuts) const
{
    ...
    if (res && res->status == 200)
    {
        try
        {
            ...
            const auto outs = j.at("outs").get<std::vector<mevacoin::RandomOuts>>();
...
std::tuple<bool, bool> Nigel::sendTransaction(
    const mevacoin::Transaction tx) const
{
    json j = {
        {"tx_as_hex", common::toHex(mevacoin::toBinaryArray(tx))}};
