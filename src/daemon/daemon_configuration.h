...
namespace DaemonConfig
{
    struct DaemonConfiguration
    {
        DaemonConfiguration()
        {
            ...
            logfile << mevacoin::MEVACOIN_NAME << "d.log";

            dataDirectory = tools::getDefaultDataDirectory();
            checkPoints = "default";
            logFile = logfile.str();
            logLevel = logging::WARNING;
            dbMaxOpenFiles = mevacoin::DATABASE_DEFAULT_MAX_OPEN_FILES;
            dbReadCacheSizeMB = mevacoin::DATABASE_READ_BUFFER_MB_DEFAULT_SIZE;
            dbThreads = mevacoin::DATABASE_DEFAULT_BACKGROUND_THREADS_COUNT;
            dbWriteBufferSizeMB = mevacoin::DATABASE_WRITE_BUFFER_MB_DEFAULT_SIZE;
            rewindToHeight = 0;
            p2pInterface = "0.0.0.0";
            p2pPort = mevacoin::P2P_DEFAULT_PORT;
            p2pExternalPort = 0;
            rpcInterface = "127.0.0.1";
            rpcPort = mevacoin::RPC_DEFAULT_PORT;
            noConsole = false;
