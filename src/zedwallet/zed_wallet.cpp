...
/* On ctrl+c the program seems to throw "zedwallet.exe has stopped
   ...
   terminate() */
    ...
    std::cout << InformationMsg(mevacoin::getProjectCLIHeader()) << std::endl;
    ...
    const mevacoin::Currency currency = mevacoin::CurrencyBuilder(logManager).currency();

    syst::Dispatcher localDispatcher;
    syst::Dispatcher *dispatcher = &localDispatcher;

    /* Our connection to mevacoind */
    std::unique_ptr<mevacoin::INode> node(
        new mevacoin::NodeRpcProxy(config.host, config.port, 10, logManager));
    ...
    */
    ...
    mevacoin::WalletGreen wallet(*dispatcher, currency, *node, logManager);
