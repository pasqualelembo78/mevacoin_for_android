...
void splitTX(mevacoin::WalletGreen &wallet,
             const mevacoin::TransactionParameters originalTXParams,
             uint32_t nodeFee)
{
              ...
              bool sendAll, std::string nodeAddress, uint32_t nodeFee)
{
    ...
    uint64_t mixin = mevacoin::getDefaultMixinByHeight(height);
    ...
    if (sendAll)
    {
        if (mevacoin::getDefaultMixinByHeight(height) != 0 && balance != balanceNoDust)
        {
                                  ...
                                  uint64_t height, uint32_t nodeFee)
{
    ...
    else if (mevacoin::getDefaultMixinByHeight(height) != 0 &&
...
void sendTX(std::shared_ptr<WalletInfo> walletInfo,
            mevacoin::TransactionParameters p, uint32_t height,
                         ...
                         uint32_t height)
{
    ...
    switch (e.code().value())
    {
    case mevacoin::error::WRONG_AMOUNT:
    {
        wrongAmount = true;
        [[fallthrough]];
    }
    case mevacoin::error::MIXIN_COUNT_TOO_BIG:
    case mevacoin::NodeError::INTERNAL_NODE_ERROR:
    {
    ...
    case mevacoin::NodeError::NETWORK_ERROR:
    case mevacoin::NodeError::CONNECT_ERROR:
    {
