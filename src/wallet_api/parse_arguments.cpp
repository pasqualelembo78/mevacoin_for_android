...
Config parseArguments(int argc, char **argv)
{
    ...
    cxxopts::Options options(argv[0], mevacoin::getProjectCLIHeader());
    ...
    options.add_options("Network")("p,port", "The port to listen on for http requests",
                                   cxxopts::value<uint16_t>(config.port)->default_value(std::to_string(mevacoin::SERVICE_DEFAULT_PORT)), "<port>")
    ...
    {
        std::cout << mevacoin::getProjectCLIHeader() << std::endl;
        exit(0);
    }
