...
void ApiDispatcher::middleware(
    ...
    const bool walletMustBeOpen,
    const bool viewWalletPermitted,
    std::function<std::tuple<Error, uint16_t>(const Request &req,
                                              Response &res,
    ...
    const nlohmann::json &body)
{
    ...
    else
    {
        ...
        std::tie(std::ignore, std::ignore, mixin) = mevacoin::Mixins::getMixinAllowableRange(
...
ApiDispatcher::getDefaultWalletParams(const nlohmann::json body) const
{
    ...
    uint16_t daemonPort = mevacoin::RPC_DEFAULT_PORT;
