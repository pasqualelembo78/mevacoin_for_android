...
void PeerlistManager::serialize(mevacoin::ISerializer &s)
{
...
void serialize(NetworkAddress &na, mevacoin::ISerializer &s)
{
    s(na.ip, "ip");
    s(na.port, "port");
}

void serialize(PeerlistEntry &pe, mevacoin::ISerializer &s)
{
    s(pe.adr, "adr");
    s(pe.id, "id");
...
PeerlistManager::PeerlistManager() : m_whitePeerlist(m_peers_white, mevacoin::P2P_LOCAL_WHITE_PEERLIST_LIMIT),
