...
namespace send_transaction
{
    ...
    {
        const auto [minMixin, maxMixin, defaultMixin] = mevacoin::Mixins::getMixinAllowableRange(
        ...
        const std::shared_ptr<SubWallets> subWallets)
    {
        ...
        if (maxFusionInputs < mevacoin::parameters::FUSION_TX_MIN_INPUT_COUNT)
        {
        ...
        while (true)
        {
            ...
            if (ourInputs.size() < mevacoin::parameters::FUSION_TX_MIN_INPUT_COUNT)
            {
            ...
            uint64_t requiredInputOutputRatio = mevacoin::parameters::FUSION_TX_MIN_IN_OUT_COUNT_RATIO;
            ...
            const uint64_t txSize = mevacoin::toBinaryArray(tx).size();

            /* Transaction is too large, remove an input and try again */
            if (txSize > mevacoin::parameters::FUSION_TX_MAX_SIZE)
            {
                /* Reduce the amount we're sending */
                foundMoney -= ourInputs.back().input.amount;
    ...
    std::tuple<Error, std::vector<mevacoin::KeyInput>, std::vector<crypto::SecretKey>> setupInputs(
        ...
        const crypto::SecretKey privateViewKey)
    {
        std::vector<mevacoin::KeyInput> inputs;
        ...
        for (const auto input : inputsAndFakes)
        {
            ...
            keyInput.outputIndexes = mevacoin::absolute_output_offsets_to_relative(keyInput.outputIndexes);
        ...
        std::vector<wallet_types::TransactionDestination> destinations)
    {
        ...
        /* Generate a random key pair for the transaction - public key gets added
           to tx extra */
        mevacoin::KeyPair randomTxKey = mevacoin::generateKeyPair();
        ...
        const std::vector<crypto::SecretKey> tmpSecretKeys)
    {
        ...
        mevacoin::getObjectHash(
            static_cast<mevacoin::TransactionPrefix>(tx), txPrefixHash);
        ...
        for (const auto input : inputsAndFakes)
        {
            ...
            const auto [success, signatures] = crypto::crypto_ops::generateRingSignatures(
                txPrefixHash, boost::get<mevacoin::KeyInput>(tx.inputs[i]).keyImage,
        ...
        for (const auto input : inputsAndFakes)
        {
            ...
            if (!crypto::crypto_ops::checkRingSignature(
                    txPrefixHash,
                    boost::get<mevacoin::KeyInput>(tx.inputs[i]).keyImage,
                    publicKeys,
                    tx.signatures[i]))
            {
                return {FAILED_TO_CREATE_RING_SIGNATURE, tx};
    ...
    {
        std::vector<mevacoin::TransactionInput> result;
    ...
    std::vector<mevacoin::TransactionOutput> keyOutputToTransactionOutput(
        const std::vector<wallet_types::KeyOutput> keyOutputs)
    {
        std::vector<mevacoin::TransactionOutput> result;

        for (const auto output : keyOutputs)
        {
            mevacoin::TransactionOutput tmpOutput;

            tmpOutput.amount = output.amount;

            mevacoin::KeyOutput tmpKey;
    ...
    {
        std::vector<uint8_t> data = mevacoin::toBinaryArray(tx);
        ...
        const uint64_t unlockTime)
    {
        ...
        /* Append the transaction public key we generated earlier to the extra
           data */
        mevacoin::addTransactionPublicKeyToExtra(extra, result.txKeyPair.publicKey);

        mevacoin::Transaction setupTX;

        setupTX.version = mevacoin::CURRENT_TRANSACTION_VERSION;
    ...
    bool verifyAmounts(const std::vector<uint64_t> amounts)
    {
        ...
        const auto prettyAmounts = mevacoin::Currency::PRETTY_AMOUNTS;
    ...
    bool verifyTransactionFee(const uint64_t expectedFee, const mevacoin::Transaction tx)
    {
        ...
        {
            inputTotal += boost::get<mevacoin::KeyInput>(input).amount;
        }
