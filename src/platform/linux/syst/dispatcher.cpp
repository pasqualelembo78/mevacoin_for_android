...
// Aggiunti per std::thread e utilità C++
#include <thread>
#include <utility>
// Detect availability of ucontext APIs
...
namespace syst
{
    ...
    namespace
    {
        ...
        static_assert(Dispatcher::SIZEOF_PTHREAD_MUTEX_T == ((int)sizeof(pthread_mutex_t)), "invalid pthread mutex size");
#endif
...
#endif
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        if (r != 0) message = "pthread_mutexattr_init failed, " + errorMessage(r);
                        else
                        {
                            r = pthread_mutexattr_settype(&attr, PTHREAD_MUTEX_NORMAL);
                            if (r != 0) message = "pthread_mutexattr_settype failed, " + errorMessage(r);
                            else
                            {
                                r = pthread_mutex_init(reinterpret_cast<pthread_mutex_t *>(this->mutex), &attr);
                                if (r != 0) message = "pthread_mutex_init failed, " + errorMessage(r);
                            }
...
#else
        // On platforms without ucontext (e.g. Android NDK) we cannot perform user-land context swap.
        // Provide a clear error so the caller knows this Dispatcher is not supported.
        throw std::runtime_error("Dispatcher::dispatch: ucontext-based context switching not supported on this platform");
#endif
    }
    ...
    void Dispatcher::spawn(std::function<void()> &&procedure)
    {
        ...
        pushContext(context);
#else
        // Platforms without ucontext: spawn executes procedure immediately in a new std::thread
        std::thread t([proc = std::move(procedure)]() mutable {
            try { proc(); } catch (...) {}
        });
        t.detach();
#endif
...
#else
        // No-op on platforms without ucontext: remote spawns already detached to threads
        return;
#endif
    }
    ...
    NativeContext &Dispatcher::getReusableContext()
    {
        ...
        return *context;
#else
        throw std::runtime_error("Dispatcher::getReusableContext: not supported on this platform");
#endif
    }
...
#else
        (void)ucontext;
        throw std::runtime_error("Dispatcher::contextProcedure: not supported on this platform");
#endif
    };
...
#else
        (void)context;
        throw std::runtime_error("Dispatcher::contextProcedureStatic: not supported on this platform");
#endif
    }
