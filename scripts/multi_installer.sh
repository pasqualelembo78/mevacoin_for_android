...
# Kryptokrona Multi-installer
# a one line clone-and-compile for mevacoin:
#
#     ` $ curl -sL "https://raw.githubusercontent.com/mevacoin/mevacoin/master/scripts/multi_installer.sh" | bash
#
...
_set_wd() {
    if [ -d "$PWD/.git" ] && [ -f "$PWD/Makefile" ]; then
        ...
        if [ -d "$PWD"/mevacoin ]; then
            read -r -p "${1:-mevacoin directory already exists. Overwrite? [y/N]} " response
            case "$response" in
                [yY][eE][sS|[yY])
                    _colorize red "Overwriting old mevacoin directory" && echo
                    rm -rf "$PWD"/mevacoin
                    ;;
                *)
                    _fail "mevacoin directory already exists. Aborting..."
                    ;;
            esac
        fi
        mkdir mevacoin
        git clone -b master -q https://github.com/mevacoin/mevacoin mevacoin   >>build.log 2>&1 || _fail "Unable to clone git repository. Please see build.log for more information"
        cd mevacoin
    fi
}

_build_mevacoin() {
    _note "Building mevacoin from source (this might take a while)..."
    if [ -d build ]; then
...
_configure_linux() {
    if [ "$(awk -F= '/^NAME/{print $2}' /etc/os-release)" = "\"Ubuntu\"" ]; then
        ...
        _fail "Your OS version isn't supported by this installer. Please consider adding support for your OS to the project ('https://github.com/mevacoin')"
    fi
}
...
_configure_os() {
    ...
    case "${_unameOut}" in
        ...
        Darwin*)
            ...
            ;;
        *)
            _fail "This installer only runs on OSX 10.10+ and Ubuntu 16.04+. Please consider adding support for your OS to the project ('https://github.com/mevacoin')"
            ;;
    esac
    _note "Operating system configuration completed. You're halfway there!"
}

_note "Kryptokrona Multi_Installer v1.0 (pepperoni)"
...
_note "Look in 'mevacoin/build/src/' for the executible binaries. See 'https://github.com/mevacoin/mevacoin' for more project support. Cowabunga!"
